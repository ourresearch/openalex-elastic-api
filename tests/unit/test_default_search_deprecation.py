...
def test_oql_renders_text_not_full_text_on_non_works():
    ...
    assert "text contains" in render(parse_url_to_oqo(entity_type="authors", search_string="x"))
    assert "full text contains" in render(parse_url_to_oqo(entity_type="works", search_string="x"))
