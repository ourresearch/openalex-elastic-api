...
DIAGNOSTICS: Dict[str, DiagnosticSpec] = {
    ...
    for s in [
        ...
        _spec("OQL_BAD_VERB_PHRASE", ERROR, PARSE,
              "after the row subject `it`, a relation verb phrase is expected",
              "use: it cites (W…), it's cited by (W…), or it's related to (W…)"),
        ...
        _spec("OQL_GROUP_VALUES_NEED_CONNECTIVE", ERROR, PARSE,
              "two values in a (…) group with no connective between them "
              "(space-adjacency is never a silent join between values)",
              "add 'or' between the values (or 'and' if you mean both)"),
        _spec("OQL_GROUP_NEEDS_ONE_VALUE", ERROR, PARSE,
              "this operator takes exactly one value in its (…) group",
              "keep one value in the parens, e.g. year >= (2019); "
              "combine several with or-clauses"),
        ...
        _spec("OQL_BAD_PROXIMITY", ERROR, PARSE,
              'malformed "within N (...)" proximity syntax',
              'e.g. title has (within 5 ("machine learning", "health"))'),
        _spec("OQL_PROXIMITY_SUFFIX_REMOVED", ERROR, PARSE,
              'proximity is written `within N (a, b, ...)` before the terms, not after',
              'e.g. within 3 ("smart", "phone")'),
        _spec("OQL_PROXIMITY_NEEDS_OPERANDS", ERROR, PARSE,
