...
class BooleanField(Field):
    def build_query(self):
        ...
        if self.param == "has_oa_accepted_or_published_version":
            ...
            # TODO(2026-03-12): Remove this workaround after E2E fixes the
            # has_abstract boolean in ES. The boolean is currently wrong for
            # ~22M Elsevier/Springer non-OA works (abstract text exists but
            # abstract_inverted_index was suppressed). This excludes them at
            # query time until the data is corrected.
            self.validate_true_false()
            suppressed = (
                Q(
                    "terms",
